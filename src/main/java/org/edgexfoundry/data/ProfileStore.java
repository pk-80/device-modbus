...
public class ProfileStore {
	...
	// profile
	// object key
	private Map<String, Map<String, ModbusObject>> objects = new HashMap<>();

	public Map<String, Map<String, ModbusObject>> getObjects() {
		return objects;
	}
	...
	public void addDevice(Device device) {
		...
		// Create a value descriptor for each parameter using its underlying
		// object
		for (ResourceOperation op : ops) {
			ValueDescriptor descriptor = descriptors.stream().filter(d -> d.getName().equals(op.getParameter()))
					.findAny().orElse(null);
			...
			if (descriptor == null) {
				...
				DeviceObject object = device.getProfile().getDeviceResources().stream()
						.filter(obj -> obj.getName().equals(op.getObject()) && obj.getProperties() != null).findAny()
						.orElse(null);
	...
	private ValueDescriptor createDescriptor(String name, DeviceObject object, Device device) {
		...
		ValueDescriptor descriptor = new ValueDescriptor(name, value.getMinimum(), value.getMaximum(),
				IoTType.valueOf(value.getType().substring(0, 1)), units.getDefaultValue(), value.getDefaultValue(),
	...
	public boolean descriptorExists(String name) {
		return !getValueDescriptors().stream().filter(desc -> desc.getName().equals(name)).collect(Collectors.toList())
				.isEmpty();
	}
}
