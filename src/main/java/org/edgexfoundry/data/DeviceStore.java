...
public class DeviceStore {
	...
	public boolean remove(String deviceId) {
		ModbusDevice d = devices.values().stream().filter(device -> device.getId().equals(deviceId)).findAny().orElse(null);
		if (d != null) {
			remove(d);
		}
		return true;
	...
	private ModbusDevice addDeviceToMetaData(ModbusDevice device) {
		...
		ModbusDevice ModbusDevice = new ModbusDevice(device);
		devices.put(device.getName(), ModbusDevice);
		return device;
	}
	...
	public Map<String, ModbusDevice> getModbusDevices() {
		return devices;
	}
	...
	public ModbusDevice getModbusDeviceById(String deviceId) {
		if (devices != null) {
			return devices.values().stream().filter(device -> device.getId().equals(deviceId)).findAny().orElse(null);
		}
		return null;
